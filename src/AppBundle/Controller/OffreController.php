...
class OffreController extends CommonController
{
    ...
    public function indexAction(Request $oRequest){
        ...
        $oQuery = $oEm->getRepository('AppBundle:Offre')->listAll();
        $aoOffres = $oPaginator->paginate(
            $oQuery, // query NOT result 
            $oRequest->query->getInt('page', 1), // page number,
            2 // limit per page
        );
        return $this->render('@App/admin/offre/index.html.twig', array(
            'offres' => $aoOffres,
        ));
    }
    ...
    public function newAction(Request $oRequest, Etablissement $oEtablissement) {
        if ($oRequest->getMethod() == 'POST'){
            ...
            $oFile = $oRequest->files->get('AppBundle_offre')['file'];
            $aRequestContent = $oRequest->request->all();
            if ($oFile instanceof UploadedFile && !empty($aRequestContent) && key_exists('AppBundle_offre', $aRequestContent)){
                $oOffre = new Offre();
                ...
                $oOffre->setNom($aRequestContent['AppBundle_offre']['nom']);
                $oOffre->setPrix($aRequestContent['AppBundle_offre']['prix']);
                $oOffre->setDescriptions($aRequestContent['AppBundle_offre']['descriptions']);
        ...
        return $this->render('@App/admin/offre/new.html.twig', ['etablissement' => $oEtablissement]);
    }
    ...
    public function showAction(Offre $oOffre){
        return $this->render('@App/admin/offre/show.html.twig', array(
            'offre' => $oOffre
        ));
    }
    ...
    public function editAction(Request $oRequest, Offre $oOffre){
        $oEditForm = $this->createForm('AppBundle\Form\OffreType', $oOffre);
        ...
        return $this->render('@App/admin/offre/edit.html.twig', array(
            'offre' => $oOffre,
